...
/* Draw the 3D effect (if configured) */
  ...
  */
  if (ColorIndex < 2) {
	...
	if(disp_in_file_dir == 1)
	{
		GUI_UC_SetEncodeNone();
	}
  if(pObj->BmpFileName != 0)
  {
  	Pic_Read(pObj->BmpFileName,bmp_public_buf);
	}
	if(pObj->gcode_path != 0)
	{
		//Pic_Read(pObj->BmpFileName,bmp_public_buf);
		drawicon_preview(pObj->gcode_path,100,100);
	}
...
/*
void BUTTON_SetBmpFileName(BUTTON_Handle hObj, char *pFile)
{
	if (hObj && pFile) {
	    BUTTON_Obj* pObj;
	    WM_LOCK();
