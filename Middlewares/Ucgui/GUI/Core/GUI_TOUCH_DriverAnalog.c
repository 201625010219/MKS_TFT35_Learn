...
static int _AD2Y(int ady) {
  ...
  r *= GUI_TOUCH_YSIZE - 1;
  return GUI_TOUCH_YSIZE -  r/(xyMinMax[GUI_COORD_Y].Max - xyMinMax[GUI_COORD_Y].Min);    
}
#endif
...
void GUI_TOUCH_Exec(void) {
  ...
  #else
  	TOUCH_X_MeasureXY((unsigned short *)&xPhys, (unsigned short *)&yPhys);
...
void setTouchBound(int x0, int x1, int y0, int y1 )
{
	//if(x0 < x1)
	{
		xyMinMax[GUI_COORD_X].Min = x0;
		xyMinMax[GUI_COORD_X].Max = x1;
	}
/*	else
	{
		xyMinMax[GUI_COORD_X].Min = x1;
		xyMinMax[GUI_COORD_X].Max = x0;
	}

	if(y0 < y1)*/
	{
		xyMinMax[GUI_COORD_Y].Min = y0;
		xyMinMax[GUI_COORD_Y].Max = y1;
	}
	/*else
	{
		xyMinMax[GUI_COORD_Y].Min = y1;
		xyMinMax[GUI_COORD_Y].Max = y0;
	}*/
