  ...
  switch (GUI_Context.DrawMode & (LCD_DRAWMODE_TRANS | LCD_DRAWMODE_XOR)) {
    case 0:
      ...
      do {
	   ...
	   #else
	LCD_setWindowArea(x, y, xsize, 1);
	//**ili9320_SetCursor(x, y);
	LCD_WriteRAM_Prepare(); /* Prepare to write GRAM */	
	for (; xsize > 0; xsize--)
	{
		
		LCD_WriteRAM((*p & (0x80 >> Diff)) ? Index1 : Index0);
		 if (++Diff == 8) {
          		Diff = 0;
			  p++;
		  }
	}
	LCD_setWindowArea(0, 0, 799, 479);
	#endif
...
/*********************************************************************
*
*       Draw Bitmap 1 BPP high speed
*/
static void  _DrawBitLine1BPP_H(int x, int y, U8 const GUI_UNI_PTR *p, int Diff, int xsize, const LCD_PIXELINDEX*pTrans) {
  LCD_PIXELINDEX Index0 = *(pTrans+0);
  LCD_PIXELINDEX Index1 = *(pTrans+1);
  x += Diff;
  switch (GUI_Context.DrawMode & (LCD_DRAWMODE_TRANS | LCD_DRAWMODE_XOR)) {
...
static void  DrawBitLine16BPP_H(int x, int y, U16 const GUI_UNI_PTR * p, int xsize, const LCD_PIXELINDEX * pTrans) {
  LCD_PIXELINDEX pixel;
  if ((GUI_Context.DrawMode & LCD_DRAWMODE_TRANS) == 0) {
    if (pTrans) {
...
void LCD_L0_DrawHLine  (int x0, int y,  int x1) {
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR) {
	...
	#else
	LCD_setWindowArea(x0, y, x1 - x0 + 1, 1);
	//**ili9320_SetCursor(x0, y);
	LCD_WriteRAM_Prepare(); /* Prepare to write GRAM */	
	for (; x0 <= x1; x0++) 
	{
		LCD_WriteRAM(LCD_COLORINDEX);
	}
	LCD_setWindowArea(0, 0, 799, 479);
	#endif
  }
...
void LCD_L0_DrawVLine  (int x, int y0,  int y1) {
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR) {
	...
	#else
	LCD_setWindowArea(x, y0, 1, y1 - y0 + 1);
	//**ili9320_SetCursor(x, y0);
	LCD_WriteRAM_Prepare(); /* Prepare to write GRAM */	
	for (; y0 <= y1; y0++)
	{
		LCD_WriteRAM(LCD_COLORINDEX);
...
void LCD_L0_FillRect(int x0, int y0, int x1, int y1) {
  ...
  #else
	int tmpX = x0;
	LCD_setWindowArea(x0, y0, x1 - x0 + 1, y1 - y0 + 1);
	//**ili9320_SetCursor(x0, y0);
	LCD_WriteRAM_Prepare(); /* Prepare to write GRAM */	
	for(; y0 <= y1; y0++)
	{
		for (tmpX = x0; tmpX <= x1; tmpX++) 
		{
			LCD_WriteRAM(LCD_COLORINDEX);
		}
	}
                       ...
                       const LCD_PIXELINDEX* pTrans)
{
  ...
  for (i=0; i<ysize; i++) {
	...
	int tmpX = x0;
  	int tmpXsize = xsize;
  	int tmpYsize = ysize;
	LCD_setWindowArea(x0, y0, xsize, ysize);
	//**ili9320_SetCursor(x0, y0);
	LCD_WriteRAM_Prepare(); /* Prepare to write GRAM */	
	for(; tmpYsize > 0; tmpYsize--)
	{
		
		switch (BitsPerPixel) {
		    case 1:
		      _DrawBitLine1BPP_H(x0, y0, pData, Diff, xsize, pTrans);
		      break;
		    #if (LCD_MAX_LOG_COLORS > 2)
		      case 2:
		        _DrawBitLine2BPP(x0, i + y0, pData, Diff, xsize, pTrans);
		    ...
		    #if (LCD_BITSPERPIXEL > 8)
		      case 16:
		        DrawBitLine16BPP_H(x0, y0, (const U16 *)pData, xsize, pTrans);
		        break;
		    #endif
		    }
