...
#include "Arduino.h"
//#include "HardwareTimer.h"  //skyblue 2006-11-26
#include "draw_ui.h"
#include "Repetier.h"
#include "iwdg.h"
...
float Printer::axisStepsPerMM[E_AXIS_ARRAY];
...
float Printer::maxAccelerationMMPerSquareSecond[E_AXIS_ARRAY];
...
float Printer::maxTravelAccelerationMMPerSquareSecond[E_AXIS_ARRAY];
...
//int32_t Printer::currentPositionSteps[E_AXIS_ARRAY];    //delta-skyblue
long Printer::currentPositionSteps[E_AXIS_ARRAY];    //delta-skyblue
...
void Endstops::update() {
    ...
    flag8_t newRead2 = 0;
#endif
if ((X_MIN_PIN > -1) && (Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_X==1))
        //if(READ(X_MIN_PIN) != Change_Repetier_Define.def_ENDSTOP_X_MIN_INVERTING)
          if(XMIN_OP != Change_Repetier_Define.def_ENDSTOP_X_MIN_INVERTING)
            newRead |= ENDSTOP_X_MIN_ID;

if ((X_MAX_PIN > -1) && (Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_X==1))
        //if(READ(X_MAX_PIN) != Change_Repetier_Define.def_ENDSTOP_X_MAX_INVERTING)
            if(XMAX_OP != Change_Repetier_Define.def_ENDSTOP_X_MAX_INVERTING)
            newRead |= ENDSTOP_X_MAX_ID;

if ((Y_MIN_PIN > -1) && (Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_Y==1))
        //if(READ(Y_MIN_PIN) != Change_Repetier_Define.def_ENDSTOP_Y_MIN_INVERTING)
          if(YMIN_OP != Change_Repetier_Define.def_ENDSTOP_Y_MIN_INVERTING)
            newRead |= ENDSTOP_Y_MIN_ID;

if ((Y_MAX_PIN > -1) && (Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_Y==1))
        //if(READ(Y_MAX_PIN) != Change_Repetier_Define.def_ENDSTOP_Y_MAX_INVERTING)
          if(YMAX_OP != Change_Repetier_Define.def_ENDSTOP_Y_MAX_INVERTING)
            newRead |= ENDSTOP_Y_MAX_ID;
		
if ((Z_MIN_PIN > -1) && (Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_Z==1))
        //if(READ(Z_MIN_PIN) != Change_Repetier_Define.def_ENDSTOP_Z_MIN_INVERTING)
          if(ZMIN_OP != Change_Repetier_Define.def_ENDSTOP_Z_MIN_INVERTING)
            newRead |= ENDSTOP_Z_MIN_ID;

if ((Z_MAX_PIN > -1) && (Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_Z ==1))
        //if(READ(Z_MAX_PIN) != Change_Repetier_Define.def_ENDSTOP_Z_MAX_INVERTING)
          if(ZMAX_OP != Change_Repetier_Define.def_ENDSTOP_Z_MAX_INVERTING)
...
#endif
//#if FEATURE_Z_PROBE
if(CfgPrinterItems.cfg_feature_z_probe == 1)
{
    ...
    //if(Z_PROBE_ON_HIGH ? READ(Z_PROBE_PIN) : !READ(Z_PROBE_PIN))
      if(Z_PROBE_ON_HIGH ? Z_PROBE_OP : !Z_PROBE_OP)
        newRead |= ENDSTOP_Z_PROBE_ID;
#endif		
}
//#endif
...
void Endstops::printLastState() {               
	Com::printF(PSTR("lastState="));
	Com::printF(Com::tColon,int(lastState));
	Com::println();
    }	
    
void Endstops:: resetLastState() {       
    lastState = 0;
    lastRead = 0;
    accumulator = 0;
...
if ((Y_MIN_PIN > -1) && (Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_Y))
{
...
void Printer::ArrayInit()
{
	Printer::axisStepsPerMM[0] = CfgPrinterItems.cfg_xaxis_steps_per_mm;
	Printer::axisStepsPerMM[1] = CfgPrinterItems.cfg_yaxis_steps_per_mm;
	Printer::axisStepsPerMM[2] = CfgPrinterItems.cfg_zaxis_steps_per_mm;
	Printer::axisStepsPerMM[3] = 1;

	Printer::maxFeedrate[0] = CfgPrinterItems.cfg_max_feedrate_x;
	Printer::maxFeedrate[1] = CfgPrinterItems.cfg_max_feedrate_y;
	Printer::maxFeedrate[2] = CfgPrinterItems.cfg_max_feedrate_z;

	Printer::maxAccelerationMMPerSquareSecond[0] = CfgPrinterItems.cfg_max_acceleration_units_per_sq_second_x;
	Printer::maxAccelerationMMPerSquareSecond[1] = CfgPrinterItems.cfg_max_acceleration_units_per_sq_second_y;
	Printer::maxAccelerationMMPerSquareSecond[2] = CfgPrinterItems.cfg_max_acceleration_units_per_sq_second_z;

	Printer::maxTravelAccelerationMMPerSquareSecond[0] = CfgPrinterItems.cfg_max_travel_acceleration_units_per_sq_second_x;
	Printer::maxTravelAccelerationMMPerSquareSecond[1] = CfgPrinterItems.cfg_max_travel_acceleration_units_per_sq_second_y;
	Printer::maxTravelAccelerationMMPerSquareSecond[2] = CfgPrinterItems.cfg_max_travel_acceleration_units_per_sq_second_z;

	Printer::homingFeedrate[0] = CfgPrinterItems.cfg_homing_feedrate_x;
	Printer::homingFeedrate[1] = CfgPrinterItems.cfg_homing_feedrate_y;
	Printer::homingFeedrate[2] = CfgPrinterItems.cfg_homing_feedrate_z;
...
if(Change_Repetier_Define.def_max_software_endstop_x == 1)
{
...
//#if max_software_endstop_y
	if(Change_Repetier_Define.def_max_software_endstop_y == 1)
	{
...
//#if max_software_endstop_z
	if(Change_Repetier_Define.def_max_software_endstop_z == 1)
	{
...
//#if DRIVE_SYSTEM == DELTA
	...
	{
    	allowed &= (z >= 0) && (z <= zLength + 0.05 + Change_Repetier_Define.def_ENDSTOP_Z_BACK_ON_HOME);
...
void Printer::setFanSpeedDirectly(uint8_t speed) {
    ...
    if(pwm_pos[Change_Repetier_Define.def_PWM_FAN1] == speed)
        return;
#if FAN_KICKSTART_TIME
    if(fanKickstart == 0 && speed > pwm_pos[Change_Repetier_Define.def_PWM_FAN1] && speed < 85)
    {
         if(pwm_pos[Change_Repetier_Define.def_PWM_FAN1]) fanKickstart = FAN_KICKSTART_TIME / 100;
         else                  fanKickstart = FAN_KICKSTART_TIME / 25;
    }
#endif
    pwm_pos[Change_Repetier_Define.def_PWM_FAN1] = speed;
#endif
}
void Printer::setFan2SpeedDirectly(uint8_t speed) {
	#if FAN2_PIN > -1 && FEATURE_FAN2_CONTROL
	if(pwm_pos[Change_Repetier_Define.def_PWM_FAN2] == speed)
		return;
	#if FAN_KICKSTART_TIME
	if(fan2Kickstart == 0 && speed > pwm_pos[Change_Repetier_Define.def_PWM_FAN2] && speed < 85)
	{
		if(pwm_pos[Change_Repetier_Define.def_PWM_FAN2]) fan2Kickstart = FAN_KICKSTART_TIME / 100;
		else                  fan2Kickstart = FAN_KICKSTART_TIME / 25;
	}
	#endif
	pwm_pos[Change_Repetier_Define.def_PWM_FAN2] = speed;
	#endif
}
...
if(CfgPrinterItems.cfg_drive_system == DELTA)
{
    ...
    transformCartesianStepsToDeltaSteps(cart, delta);   //delta - err1
    maxDeltaPositionSteps = delta[0];
    xMaxSteps = yMaxSteps = zMaxSteps;
    xMinSteps = yMinSteps = zMinSteps = 0;
    deltaFloorSafetyMarginSteps = Change_Repetier_Define.def_DELTA_FLOOR_SAFETY_MARGIN_MM * axisStepsPerMM[Z_AXIS];
}
/*
...
//#if HAVE_HEATED_BED
	if(CfgPrinterItems.cfg_have_heated_bed == 1)
	{
    	if(heatedBedController.targetTemperatureC < 15)      // turn off FAN_BOARD only if bed heater is off
    		pwm_pos[Change_Repetier_Define.def_PWM_BOARD_FAN] = 0;
	}
//#endif
	else
       pwm_pos[Change_Repetier_Define.def_PWM_BOARD_FAN] = 0;
...
uint8_t Printer::moveToReal(float x, float y, float z, float e, float f,bool pathOptimize)
{
	...
	if(CfgPrinterItems.cfg_ext_mintemp > 30)
	{
	    if(e != IGNORE_COORDINATE && !Printer::debugDryrun()
	       && (Extruder::current->tempControl.currentTemperatureC > CfgPrinterItems.cfg_ext_mintemp 
...
//                p = 0;
			if(CfgPrinterItems.cfg_ext_mintemp > 20)
			{
            if((Extruder::current->tempControl.currentTemperatureC < CfgPrinterItems.cfg_ext_mintemp && !Printer::isColdExtrusionAllowed())||
                fabs(com->E) * extrusionFactor > EXTRUDE_MAXLENGTH)
                p = 0;
			}
			else
...
//                (Extruder::current->tempControl.currentTemperatureC < MIN_EXTRUDER_TEMP  && !Printer::isColdExtrusionAllowed()) ||
//#endif
//                fabs((long double)(p - currentPositionSteps[E_AXIS])) * extrusionFactor > EXTRUDE_MAXLENGTH * axisStepsPerMM[E_AXIS])
//                currentPositionSteps[E_AXIS] = p;
			...
			{
				if(fabs((long double)(p - currentPositionSteps[E_AXIS])) * extrusionFactor > EXTRUDE_MAXLENGTH * axisStepsPerMM[E_AXIS])
				{
					currentPositionSteps[E_AXIS] = p;
				}
			}
...
void Printer::setup()
{
    ...
    //XENA_OP = !X_ENABLE_ON;
    XENA_OP = !CfgPrinterItems.cfg_x_enable_on;
#endif
#if Y_ENABLE_PIN > -1
    SET_OUTPUT(Y_ENABLE_PIN);
    //WRITE(Y_ENABLE_PIN, !Y_ENABLE_ON);
    YENA_OP = !CfgPrinterItems.cfg_y_enable_on;
#endif
#if Z_ENABLE_PIN > -1
    SET_OUTPUT(Z_ENABLE_PIN);
    //WRITE(Z_ENABLE_PIN, !Z_ENABLE_ON);
    ZENA_OP = !CfgPrinterItems.cfg_z_enable_on;
#endif
	...
	XENA2_OP = !CfgPrinterItems.cfg_x_enable_on;
#endif
#endif
	...
	YENA2_OP = !CfgPrinterItems.cfg_y_enable_on;
#endif
#endif
    ...
    ZENA2_OP = !CfgPrinterItems.cfg_z_enable_on;
#endif
#endif
    ...
    ZENA3_OP = !CfgPrinterItems.cfg_z_enable_on;
#endif
#endif

    //end stop pull ups
//#if MIN_HARDWARE_ENDSTOP_X
if(Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_X == 1)
{
#if X_MIN_PIN > -1
    SET_INPUT(X_MIN_PIN);
...
if(Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_Y==1)
{
#if Y_MIN_PIN > -1
    SET_INPUT(Y_MIN_PIN);
...
if(Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_Z==1)
{
#if Z_MIN_PIN > -1
    SET_INPUT(Z_MIN_PIN);
...
if (Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_X==1)
{
#if X_MAX_PIN > -1
    SET_INPUT(X_MAX_PIN);
...
//#endif
if (Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_Y==1)
{
#if Y_MAX_PIN > -1
    SET_INPUT(Y_MAX_PIN);
...
if(Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_Z == 1)
{
#if Z_MAX_PIN>-1
    SET_INPUT(Z_MAX_PIN);
...
#endif
#if defined(EXT1_EXTRUDER_COOLER_PIN) && EXT1_EXTRUDER_COOLER_PIN > -1 
	if(CfgPrinterItems.cfg_num_extruder > 1)
		{
...
#endif
#if defined(EXT2_EXTRUDER_COOLER_PIN) && EXT2_EXTRUDER_COOLER_PIN > -1 
	if(CfgPrinterItems.cfg_num_extruder > 2)
	{
...
#endif
#if defined(EXT3_EXTRUDER_COOLER_PIN) && EXT3_EXTRUDER_COOLER_PIN > -1
	if(CfgPrinterItems.cfg_num_extruder > 3)
		{
...
#endif
#if defined(EXT4_EXTRUDER_COOLER_PIN) && EXT4_EXTRUDER_COOLER_PIN > -1 
	if(CfgPrinterItems.cfg_num_extruder > 4)
	{
...
#endif
#if defined(EXT5_EXTRUDER_COOLER_PIN) && EXT5_EXTRUDER_COOLER_PIN > -1 
	if(CfgPrinterItems.cfg_num_extruder > 5)
	{
...
//#if GANTRY && !defined(FAST_COREXYZ)
	if( CfgPrinterItems.cfg_drive_system ==XY_GANTRY 
		|| CfgPrinterItems.cfg_drive_system ==YX_GANTRY 
		|| CfgPrinterItems.cfg_drive_system ==XZ_GANTRY 
		|| CfgPrinterItems.cfg_drive_system ==ZX_GANTRY 
		|| CfgPrinterItems.cfg_drive_system == GANTRY_FAKE)
	{
...
//#endif
    ...
    advanceStepsSet = 0;
#endif
    for(uint8_t i = 0; i < CfgPrinterItems.cfg_num_extruder + 3; i++) pwm_pos[i] = 0;
    maxJerk = MAX_JERK;
//#if DRIVE_SYSTEM != DELTA
	if(CfgPrinterItems.cfg_drive_system != DELTA)
	{
...
//#endif
    ...
    //yMin = Y_MIN_POS;
    //zMin = Z_MIN_POS;
    xLength = CfgPrinterItems.cfg_x_max_length;
    yLength = CfgPrinterItems.cfg_y_max_length;
    zLength = CfgPrinterItems.cfg_z_max_length;
    xMin = CfgPrinterItems.cfg_x_min_pos;
    yMin = CfgPrinterItems.cfg_y_min_pos;
    zMin = CfgPrinterItems.cfg_z_min_pos;
//#if DRIVE_SYSTEM == DELTA
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
    	radius0 = CfgPrinterItems.cfg_rod_radius;//(Change_Repetier_Define.def_PRINTER_RADIUS-Change_Repetier_Define.def_END_EFFECTOR_HORIZONTAL_OFFSET-Change_Repetier_Define.def_CARRIAGE_HORIZONTAL_OFFSET);//ROD_RADIUS;
	}
//#endif
#if ENABLE_BACKLASH_COMPENSATION
...
//#if NONLINEAR_SYSTEM            //delta - err2
if(Change_Repetier_Define.def_NONLINEAR_SYSTEM == 1)
{
    transformCartesianStepsToDeltaSteps(Printer::currentPositionSteps, Printer::currentNonlinearPositionSteps);

//#if DELTA_HOME_ON_POWER
if(Change_Repetier_Define.def_DELTA_HOME_ON_POWER == 1)
{
    homeAxis_delta(true,true,true);
}
//#endif
...
{
  volatile float mksDeltaDiagonalRodLength;
    ...
    mksDeltaDiagonalRodLength = EEPROM::deltaDiagonalRodLength();
    PrintLine::moveRelativeDistanceInSteps(0, 0, (zMaxSteps + mksDeltaDiagonalRodLength*axisStepsPerMM[Z_AXIS]) * 1.5, 0, feedrate, true, true);
    offsetX = offsetY = offsetZ = 0;
    setHoming(false);
}
void Printer::homeXAxis_delta()
{
...
void Printer::homeZAxis_delta() // Delta z homing
{
	...
	if (Endstops::xMax() && Endstops::yMax() && Endstops::zMax()) {
		// Back off for retest
		PrintLine::moveRelativeDistanceInSteps(0, 0, axisStepsPerMM[Z_AXIS] * -Change_Repetier_Define.def_ENDSTOP_Z_BACK_MOVE, 0, Printer::homingFeedrate[Z_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR, true, true);
		//Endstops::report();
...
//#if defined(ENDSTOP_Z_BACK_ON_HOME)
				if(Change_Repetier_Define.def_ENDSTOP_Z_BACK_ON_HOME > 0) {
					...
					PrintLine::moveRelativeDistanceInSteps(0, 0, axisStepsPerMM[Z_AXIS] * -Change_Repetier_Define.def_ENDSTOP_Z_BACK_ON_HOME * CfgPrinterItems.cfg_z_home_dir,0,homingFeedrate[Z_AXIS], true, true);
					//Endstops::report();
...
//#if defined(ENDSTOP_Z_BACK_ON_HOME)
    if(Change_Repetier_Define.def_ENDSTOP_Z_BACK_ON_HOME > 0)
        maxDeltaPositionSteps += axisStepsPerMM[Z_AXIS] * Change_Repetier_Define.def_ENDSTOP_Z_BACK_ON_HOME;
//#endif
    Extruder::selectExtruderById(Extruder::current->id);
}
// This home axis is for delta
void Printer::homeAxis_delta(bool xaxis,bool yaxis,bool zaxis) // Delta homing code
{
    bool autoLevel = isAutolevelActive();
    setAutolevelActive(false);
    setHomed(true);
    if (!(X_MAX_PIN > -1 && Y_MAX_PIN > -1 && Z_MAX_PIN > -1
            && (Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_X==1) && (Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_Y==1) && (Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_Z==1)))
    {
...
void Printer::homeXAxis()
{
    ...
    if (((Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_X == 1) && X_MIN_PIN > -1 && CfgPrinterItems.cfg_x_home_dir== -1 && (Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_Y==1) && Y_MIN_PIN > -1 && CfgPrinterItems.cfg_y_home_dir == -1) ||
            ((Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_X==1) && X_MAX_PIN > -1 && CfgPrinterItems.cfg_x_home_dir == 1 && (Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_Y==1) && Y_MAX_PIN > -1 && CfgPrinterItems.cfg_y_home_dir == 1))
...
//#if NUM_EXTRUDER>1
        ...
        {
//#if X_HOME_DIR < 0
if(CfgPrinterItems.cfg_x_home_dir < 0)
{
...
//#endif
        ...
        steps = (Printer::xMaxSteps-Printer::xMinSteps) * CfgPrinterItems.cfg_x_home_dir;
        ...
        currentPositionSteps[X_AXIS] = (CfgPrinterItems.cfg_x_home_dir == -1) ? xMinSteps-offX : xMaxSteps+offX;
        ...
        PrintLine::moveRelativeDistanceInSteps(axisStepsPerMM[X_AXIS]*-Change_Repetier_Define.def_ENDSTOP_X_BACK_MOVE * CfgPrinterItems.cfg_x_home_dir,0,0,0,homingFeedrate[X_AXIS]/ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,false);
        PrintLine::moveRelativeDistanceInSteps(axisStepsPerMM[X_AXIS]*2*Change_Repetier_Define.def_ENDSTOP_X_BACK_MOVE * CfgPrinterItems.cfg_x_home_dir,0,0,0,homingFeedrate[X_AXIS]/ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,true);
		setHoming(false);
//#if defined(ENDSTOP_X_BACK_ON_HOME)
        //if(ENDSTOP_X_BACK_ON_HOME > 0)
        if(Change_Repetier_Define.def_ENDSTOP_BACK_ON_HOME > 0)
        {
            ...
            PrintLine::moveRelativeDistanceInSteps(axisStepsPerMM[X_AXIS]*-Change_Repetier_Define.def_ENDSTOP_BACK_ON_HOME * CfgPrinterItems.cfg_x_home_dir,0,0,0,homingFeedrate[X_AXIS],true,false);
...
if(CfgPrinterItems.cfg_num_extruder>1)
{
        ...
        PrintLine::moveRelativeDistanceInSteps((Extruder::current->xOffset-offX) * CfgPrinterItems.cfg_x_home_dir,(Extruder::current->yOffset-offY) * Y_HOME_DIR,0,0,homingFeedrate[X_AXIS],true,false);
//#endif
}
    }
}
...
if((DUAL_X_AXIS) && (CfgPrinterItems.cfg_num_extruder == 2))
{
    ...
    PrintLine::moveRelativeDistanceInSteps(-2 * steps, 0, 0, 0, homingFeedrate[X_AXIS], true, true);
    setHoming(false);
	PrintLine::moveRelativeDistanceInSteps(axisStepsPerMM[X_AXIS] * Change_Repetier_Define.def_ENDSTOP_X_BACK_MOVE,0,0,0,homingFeedrate[X_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR, true, false);
    setHoming(true);	
    PrintLine::moveRelativeDistanceInSteps(-axisStepsPerMM[X_AXIS] * 2 * Change_Repetier_Define.def_ENDSTOP_X_BACK_MOVE,0,0,0,homingFeedrate[X_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR, true, true);
    setHoming(false);
//#if defined(ENDSTOP_X_BACK_ON_HOME)
    if(Change_Repetier_Define.def_ENDSTOP_X_BACK_ON_HOME > 0)
    {
        PrintLine::moveRelativeDistanceInSteps(axisStepsPerMM[X_AXIS] * Change_Repetier_Define.def_ENDSTOP_X_BACK_ON_HOME,0,0,0,homingFeedrate[X_AXIS], true, false);
    }
//#endif
...
else	
{
    ...
    if (((Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_X==1) && X_MIN_PIN > -1 && CfgPrinterItems.cfg_x_home_dir== -1) || ((Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_X==1) && X_MAX_PIN > -1 && CfgPrinterItems.cfg_x_home_dir == 1))
    {
		coordinateOffset[X_AXIS] = 0;
        long offX = 0;
//#if NUM_EXTRUDER > 1
...
//#if defined(ENDSTOP_X_BACK_ON_HOME)
        if(Change_Repetier_Define.def_ENDSTOP_X_BACK_ON_HOME > 0)
            ...
            PrintLine::moveRelativeDistanceInSteps(axisStepsPerMM[X_AXIS] * -Change_Repetier_Define.def_ENDSTOP_X_BACK_ON_HOME * CfgPrinterItems.cfg_x_home_dir,0,0,0,homingFeedrate[X_AXIS], true, true);
//#endif
        //currentPositionSteps[X_AXIS] = (X_HOME_DIR == -1) ? xMinSteps - offX : xMaxSteps + offX;
        currentPositionSteps[X_AXIS] = (CfgPrinterItems.cfg_x_home_dir == -1) ? xMinSteps - offX : xMaxSteps + offX;
//#if NONLINEAR_SYSTEM
...
if(CfgPrinterItems.cfg_x_home_dir < 0)
        ...
        PrintLine::moveRelativeDistanceInSteps((Extruder::current->xOffset - offX) * CfgPrinterItems.cfg_x_home_dir,0,0,0,homingFeedrate[X_AXIS], true, true);
//#else
else
        //PrintLine::moveRelativeDistanceInSteps(-(Extruder::current->xOffset - offX) * X_HOME_DIR,0,0,0,homingFeedrate[X_AXIS], true, true);
        PrintLine::moveRelativeDistanceInSteps(-(Extruder::current->xOffset - offX) * CfgPrinterItems.cfg_x_home_dir,0,0,0,homingFeedrate[X_AXIS], true, true);
...
void Printer::homeYAxis()
{
    ...
    if (((Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_Y==1) && Y_MIN_PIN > -1 && CfgPrinterItems.cfg_y_home_dir== -1) || ((Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_Y==1) && Y_MAX_PIN > -1 && CfgPrinterItems.cfg_y_home_dir == 1))
    {
		coordinateOffset[Y_AXIS] = 0;
        long offY = 0;
//#if NUM_EXTRUDER > 1
if(CfgPrinterItems.cfg_num_extruder>1)
{
        for(uint8_t i = 0; i < CfgPrinterItems.cfg_num_extruder; i++)
//#if Y_HOME_DIR < 0
if(CfgPrinterItems.cfg_y_home_dir < 0)
            offY = RMath::max(offY,extruder[i].yOffset);
//#else
else
            offY = RMath::min(offY,extruder[i].yOffset);
...
//#endif
        ...
        steps = (yMaxSteps-Printer::yMinSteps) * CfgPrinterItems.cfg_y_home_dir;
...
//#endif
        ...
        currentPositionSteps[Y_AXIS] = (CfgPrinterItems.cfg_y_home_dir == -1) ? yMinSteps-offY : yMaxSteps+offY;
//#if NONLINEAR_SYSTEM
		if(Change_Repetier_Define.def_NONLINEAR_SYSTEM == 1)
		{
			transformCartesianStepsToDeltaSteps(currentPositionSteps, currentNonlinearPositionSteps);
...
//        PrintLine::moveRelativeDistanceInSteps(0,axisStepsPerMM[Y_AXIS] * 2 * ENDSTOP_Y_BACK_MOVE * Y_HOME_DIR,0,0,homingFeedrate[Y_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,true);
        PrintLine::moveRelativeDistanceInSteps(0,axisStepsPerMM[Y_AXIS] * -Change_Repetier_Define.def_ENDSTOP_Y_BACK_MOVE * CfgPrinterItems.cfg_y_home_dir,0,0,homingFeedrate[Y_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,false);
        PrintLine::moveRelativeDistanceInSteps(0,axisStepsPerMM[Y_AXIS] * 2 * Change_Repetier_Define.def_ENDSTOP_Y_BACK_MOVE * CfgPrinterItems.cfg_y_home_dir,0,0,homingFeedrate[Y_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,true);
        setHoming(false);
//#if defined(ENDSTOP_Y_BACK_ON_HOME)
        if(Change_Repetier_Define.def_ENDSTOP_Y_BACK_ON_HOME > 0)
            ...
            PrintLine::moveRelativeDistanceInSteps(0,axisStepsPerMM[Y_AXIS] * -Change_Repetier_Define.def_ENDSTOP_Y_BACK_ON_HOME  * CfgPrinterItems.cfg_y_home_dir,0,0,homingFeedrate[Y_AXIS],true,false);
//#endif
        //currentPositionSteps[Y_AXIS] = (Y_HOME_DIR == -1) ? yMinSteps - offY : yMaxSteps + offY;
        currentPositionSteps[Y_AXIS] = (CfgPrinterItems.cfg_y_home_dir == -1) ? yMinSteps - offY : yMaxSteps + offY;
//#if NONLINEAR_SYSTEM
...
if(CfgPrinterItems.cfg_y_home_dir < 0)
        ...
        PrintLine::moveRelativeDistanceInSteps(0,(Extruder::current->yOffset - offY) * CfgPrinterItems.cfg_y_home_dir,0,0,homingFeedrate[Y_AXIS],true,false);
//#else
else
        //PrintLine::moveRelativeDistanceInSteps(0,-(Extruder::current->yOffset - offY) * Y_HOME_DIR,0,0,homingFeedrate[Y_AXIS],true,false);
        PrintLine::moveRelativeDistanceInSteps(0,-(Extruder::current->yOffset - offY) * CfgPrinterItems.cfg_y_home_dir,0,0,homingFeedrate[Y_AXIS],true,false);
...
void Printer::homeZAxis() // Cartesian homing
{
    ...
    if (((Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_Z==1) && Z_MIN_PIN > -1 && CfgPrinterItems.cfg_z_home_dir== -1) || ((Change_Repetier_Define.def_MAX_HARDWARE_ENDSTOP_Z ==1) && Z_MAX_PIN > -1 && CfgPrinterItems.cfg_z_home_dir == 1))
    {
		coordinateOffset[Z_AXIS] = 0;
        UI_STATUS_UPD_F(Com::translatedF(UI_TEXT_HOME_Z_ID));
        //steps = (zMaxSteps - zMinSteps) * Z_HOME_DIR;
        steps = (zMaxSteps - zMinSteps) * CfgPrinterItems.cfg_z_home_dir;
...
//#endif
        ...
        currentPositionSteps[Z_AXIS] = (CfgPrinterItems.cfg_z_home_dir == -1) ? zMinSteps : zMaxSteps;
//#if NONLINEAR_SYSTEM
		if(Change_Repetier_Define.def_NONLINEAR_SYSTEM == 1)
		{
			transformCartesianStepsToDeltaSteps(currentPositionSteps, currentNonlinearPositionSteps);
...
//#endif
        ...
        PrintLine::moveRelativeDistanceInSteps(0,0,axisStepsPerMM[Z_AXIS] * -Change_Repetier_Define.def_ENDSTOP_Z_BACK_MOVE * CfgPrinterItems.cfg_z_home_dir,0,homingFeedrate[Z_AXIS] / ENDSTOP_Z_RETEST_REDUCTION_FACTOR,true,false);
        ...
        PrintLine::moveRelativeDistanceInSteps(0,0,axisStepsPerMM[Z_AXIS] * 2 * Change_Repetier_Define.def_ENDSTOP_Z_BACK_MOVE * CfgPrinterItems.cfg_z_home_dir,0,homingFeedrate[Z_AXIS] / ENDSTOP_Z_RETEST_REDUCTION_FACTOR,true,true);
        setHoming(false);
...
if((CfgPrinterItems.cfg_z_home_dir<0)&&(Change_Repetier_Define.def_MIN_HARDWARE_ENDSTOP_Z==1)&&(CfgPrinterItems.cfg_feature_z_probe==1))
...
//#if defined(ENDSTOP_Z_BACK_ON_HOME)
        ...
        if(Change_Repetier_Define.def_ENDSTOP_Z_BACK_ON_HOME > 0)
			...
			zCorrection -= axisStepsPerMM[Z_AXIS] * Change_Repetier_Define.def_ENDSTOP_Z_BACK_ON_HOME * CfgPrinterItems.cfg_z_home_dir;
//#endif
//#if Z_HOME_DIR < 0
if(CfgPrinterItems.cfg_z_home_dir<0)
		// Fix bed coating
...
//#endif
        ...
        currentPositionSteps[Z_AXIS] = ((CfgPrinterItems.cfg_z_home_dir == -1) ? zMinSteps : zMaxSteps - Printer::zBedOffset * axisStepsPerMM[Z_AXIS]);
//#if NUM_EXTRUDER > 0
...
if(CfgPrinterItems.cfg_homing_order == HOME_ORDER_XYZ)
{
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
    	if(xaxis) homeXAxis_delta();
    	if(yaxis) homeYAxis_delta();
    	if(zaxis) homeZAxis_delta();
	}
	else
	{
	    if(xaxis) homeXAxis();
...
else if(CfgPrinterItems.cfg_homing_order == HOME_ORDER_XZY)
{
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
	    if(xaxis) homeXAxis_delta();
...
else if(CfgPrinterItems.cfg_homing_order == HOME_ORDER_YXZ)
{
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
	    if(yaxis) homeYAxis_delta();
...
else if(CfgPrinterItems.cfg_homing_order == HOME_ORDER_YZX)
{
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
	    if(yaxis) homeYAxis_delta();
...
else if(CfgPrinterItems.cfg_homing_order == HOME_ORDER_ZXY)
{
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
    	if(zaxis) homeZAxis_delta();
...
else if(CfgPrinterItems.cfg_homing_order == HOME_ORDER_ZYX)
{
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
	    if(zaxis) homeZAxis_delta();
...
else if((CfgPrinterItems.cfg_homing_order == HOME_ORDER_ZXYTZ) || (CfgPrinterItems.cfg_homing_order == HOME_ORDER_XYTZ))
{
    //float actTemp[NUM_EXTRUDER];
    float actTemp[6];
    for(int i = 0;i < CfgPrinterItems.cfg_num_extruder; i++)
        actTemp[i] = extruder[i].tempControl.targetTemperatureC;
    if(zaxis) {
//#if HOMING_ORDER == HOME_ORDER_ZXYTZ	
if(CfgPrinterItems.cfg_homing_order == HOME_ORDER_ZXYTZ)
{
		if(CfgPrinterItems.cfg_drive_system == DELTA)
		{
			homeZAxis_delta();
...
//#if ZHOME_X_POS == IGNORE_COORDINATE
        ...
        if(CfgPrinterItems.cfg_x_home_dir< 0) startX = Printer::xMin;
...
//#if ZHOME_Y_POS == IGNORE_COORDINATE
        ...
        if(CfgPrinterItems.cfg_y_home_dir< 0) startY = Printer::yMin;
...
#if DISTORTION_CORRECTION && Z_PROBE_PIN == Z_MIN_PIN
if(CfgPrinterItems.cfg_z_home_dir < 0)
{
        ...
        if(CfgPrinterItems.cfg_z_home_dir < 0) startZ = Printer::zMin;
...
//tan 20161128
    ...
    HAL::delayMicroseconds(CfgPrinterItems.cfg_stepper_high_delay+2);
    Printer::endXYZSteps();
    HAL::delayMicroseconds(10);
//#if DRIVE_SYSTEM == 3
	if(CfgPrinterItems.cfg_drive_system == DELTA)
...
void Printer::showConfiguration() {
...
#endif
    Com::config(PSTR("NumExtruder:"),CfgPrinterItems.cfg_num_extruder);
    Com::config(PSTR("MixingExtruder:"),MIXING_EXTRUDER);
    Com::config(PSTR("HeatedBed:"),CfgPrinterItems.cfg_have_heated_bed);
    ...
    Com::config(PSTR("XHomeDir:"),CfgPrinterItems.cfg_x_home_dir);
    //Com::config(PSTR("YHomeDir:"),Y_HOME_DIR);
    Com::config(PSTR("YHomeDir:"),CfgPrinterItems.cfg_y_home_dir);
    //Com::config(PSTR("ZHomeDir:"),Z_HOME_DIR);
    Com::config(PSTR("ZHomeDir:"),CfgPrinterItems.cfg_z_home_dir);
    ...
    Com::config(PSTR("ZProbe:"),CfgPrinterItems.cfg_feature_z_probe);
...
//#endif // DRIVE_SYSTEM
    ...
    Com::config(PSTR("MaxBedTemp:"), CfgPrinterItems.cfg_heated_bed_maxtemp);
    for(fast8_t i = 0; i < CfgPrinterItems.cfg_num_extruder; i++) {
        ...
        Com::printFLN(PSTR("MaxTemp:"),CfgPrinterItems.cfg_ext_maxtemp);
    }
}
...
//#if DRIVE_SYSTEM == DELTA
			...
			if(CfgPrinterItems.cfg_drive_system == DELTA)
			{
            	mtx = Printer::invAxisStepsPerMM[X_AXIS] * (ix * step - radiusCorrectionSteps);
            	mty = Printer::invAxisStepsPerMM[Y_AXIS] * (iy * step - radiusCorrectionSteps);
			}
//#else
			else
			{
				mtx = Printer::invAxisStepsPerMM[X_AXIS] * (ix * xCorrectionSteps + xOffsetSteps);
				mty = Printer::invAxisStepsPerMM[Y_AXIS] * (iy * yCorrectionSteps + yOffsetSteps);
			}
//#endif			
...
if(CfgPrinterItems.cfg_drive_system == DELTA)
{
    ...
    fxFloor = (x - (x < 0 ? step - 1 : 0)) / step; // special case floor for negative integers!
    fyFloor = (y - (y < 0 ? step -1 : 0)) / step;
}
//#else
else
{
    x -= xOffsetSteps;
    y -= yOffsetSteps;
    fxFloor = (x - (x < 0 ? xCorrectionSteps - 1 : 0)) / xCorrectionSteps; // special case floor for negative integers!
    fyFloor = (y - (y < 0 ? yCorrectionSteps -1 : 0)) / yCorrectionSteps;
}
//#endif	
...
//#if DRIVE_SYSTEM == DELTA
	...
	int32_t zx1;
	int32_t zx2;
	int32_t correction_z;
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
	    fx = x - fxFloor * step; // Grid normalized coordinates
	    fy = y - fyFloor * step;

	    idx11 = matrixIndex(fxFloor, fyFloor);
	    m11 = getMatrix(idx11);
		m12 = getMatrix(idx11 + 1);
	    m21 = getMatrix(idx11 + DISTORTION_CORRECTION_POINTS);
	    m22 = getMatrix(idx11 + DISTORTION_CORRECTION_POINTS + 1);
	    zx1 = m11 + ((m12 - m11) * fx) / step;
	    zx2 = m21 + ((m22 - m21) * fx) / step;
	    correction_z = zx1 + ((zx2 - zx1) * fy) / step;
	}
//#else
	else
	{
	    fx = x - fxFloor * xCorrectionSteps; // Grid normalized coordinates
	    fy = y - fyFloor * yCorrectionSteps;

	    idx11 = matrixIndex(fxFloor, fyFloor);
	    m11 = getMatrix(idx11), m12 = getMatrix(idx11 + 1);
	    m21 = getMatrix(idx11 + DISTORTION_CORRECTION_POINTS);
	    m22 = getMatrix(idx11 + DISTORTION_CORRECTION_POINTS + 1);
	    zx1 = m11 + ((m12 - m11) * fx) / xCorrectionSteps;
	    zx2 = m21 + ((m22 - m21) * fx) / xCorrectionSteps;
	    correction_z = zx1 + ((zx2 - zx1) * fy) / yCorrectionSteps;
	}
//#endif
    if(false && z == 0) {
...
//#if DRIVE_SYSTEM == DELTA
	...
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
		ix = (x * Printer::axisStepsPerMM[Z_AXIS] + radiusCorrectionSteps + step / 2) / step;
		iy = (y * Printer::axisStepsPerMM[Z_AXIS] + radiusCorrectionSteps + step / 2) / step;
	}
//#else
	else
	{
		ix = (x * Printer::axisStepsPerMM[X_AXIS] - xOffsetSteps + xCorrectionSteps / 2) / xCorrectionSteps;
		iy = (y * Printer::axisStepsPerMM[Y_AXIS] - yOffsetSteps + yCorrectionSteps / 2) / yCorrectionSteps;
	}
//#endif	
	if(ix < 0) ix = 0;
	if(iy < 0) iy = 0;
...
//#if DRIVE_SYSTEM == DELTA
			...
			if(CfgPrinterItems.cfg_drive_system == DELTA)
			{
				x = (-radiusCorrectionSteps + ix * step) * Printer::invAxisStepsPerMM[Z_AXIS];
				y = (-radiusCorrectionSteps + iy * step) * Printer::invAxisStepsPerMM[Z_AXIS];
			}
//#else
			else
			{
				x = (xOffsetSteps + ix * xCorrectionSteps) * Printer::invAxisStepsPerMM[X_AXIS];
				y = (yOffsetSteps + iy * yCorrectionSteps) * Printer::invAxisStepsPerMM[Y_AXIS];
			}
//#endif			
			int32_t idx = matrixIndex(ix, iy);
...
relativeCoordinateMode = false;  ///< Determines absolute (false) or relative Coordinates (true).
relativeExtruderCoordinateMode = false;  ///< Determines Absolute or Relative E Codes while in Absolute Coordinates mode. E is always relative in Relative Coordinates mode.
...
//float Printer::lastCmdPos[Z_AXIS_ARRAY];
//int32_t Printer::destinationSteps[E_AXIS_ARRAY];
//coordinateOffset[Z_AXIS_ARRAY] = {0,0,0};
flag0 = 0;
flag1 = 0;
flag2 = 0;
debugLevel = 6; ///< Bitfield defining debug output. 1 = echo, 2 = info, 4 = error, 8 = dry run., 16 = Only communication, 32 = No moves
stepsPerTimerCall = 1;
menuMode = 0;
mode = DEFAULT_PRINTER_MODE;
fanSpeed = 0; // Last fan speed set with M106/M107
extrudeMultiplyError = 0;
extrusionFactor = 1.0;
interruptEvent = 0;
/*
#if EEPROM_MODE != 0
zBedOffset = HAL::eprGetFloat(EPR_Z_PROBE_Z_OFFSET);
#else
float Printer::zBedOffset = Z_PROBE_Z_OFFSET;
#endif
...
#endif
*/
interval = 30000;           ///< Last step duration in ticks.
/*
uint32_t Printer::timer;              ///< used for acceleration/deceleration timing
uint32_t Printer::stepNumber;         ///< Step number in current move.
#if USE_ADVANCE
...
#endif
*/
memoryX = IGNORE_COORDINATE;
memoryY = IGNORE_COORDINATE;
memoryZ = IGNORE_COORDINATE;
memoryE = IGNORE_COORDINATE;
memoryF = -1;
/*
#if GANTRY && !defined(FAST_COREXYZ)
int8_t Printer::motorX;
...
//#if HAVE_HEATED_BED
if(CfgPrinterItems.cfg_have_heated_bed)
{
...
//#elif (DRIVE_SYSTEM == CARTESIAN || DRIVE_SYSTEM == GANTRY_FAKE)
			else if((CfgPrinterItems.cfg_drive_system == CARTESIAN)||(CfgPrinterItems.cfg_drive_system == GANTRY_FAKE))
            Com::printF(PSTR("cartesian"));
//#elif ((DRIVE_SYSTEM == XY_GANTRY) || (DRIVE_SYSTEM == YX_GANTRY))
			else if((CfgPrinterItems.cfg_drive_system == XY_GANTRY)||(CfgPrinterItems.cfg_drive_system == YX_GANTRY))
            Com::printF(PSTR("coreXY"));			
//#elif (DRIVE_SYSTEM == XZ_GANTRY)
			else if(CfgPrinterItems.cfg_drive_system == XZ_GANTRY)
...
//#endif
        ...
        case 5:
            ...
            Com::print((int) CfgPrinterItems.cfg_x_min_pos);
            Com::print(',');
            //Com::print((int) Y_MIN_POS);
            Com::print((int) CfgPrinterItems.cfg_y_min_pos);
            Com::print(',');
            //Com::print((int) Z_MIN_POS);
            Com::print((int) CfgPrinterItems.cfg_z_min_pos);
            Com::printF(PSTR("],\"axisMaxes\":["));
            //Com::print((int) X_MAX_LENGTH);
            Com::print((int) CfgPrinterItems.cfg_x_max_length);
            Com::print(',');
            //Com::print((int) Y_MAX_LENGTH);
            Com::print((int) CfgPrinterItems.cfg_y_max_length);
            Com::print(',');
            //Com::print((int) Z_MAX_LENGTH);
            Com::print((int) CfgPrinterItems.cfg_z_max_length);
