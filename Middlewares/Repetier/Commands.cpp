...
#include "wiring_digital.h"        //skyblue 2006-11-26
#include "wiring_analog.h"        //skyblue 2006-11-26
...
#include "fatfs.h"
//extern "C" unsigned int   getTick();

//extern "C" unsigned int  getTickDiff(unsigned int curTick, unsigned int  lastTick);
...
//extern volatile uint8_t sd_continue_flag;
//extern volatile uint8_t sd_stop_flag;
//extern volatile uint8_t sd_operate_flag;
extern volatile uint8_t Back_to_Printing;
extern void Close_machine_signal();
...
void Commands::commandLoop() {
    ...
    /*
      mksLedCnt++;
        if(mksLedCnt % 100000 == 0)
          digitalWrite(46,!digitalRead(46));
    */	
    	//tan 控温错误
    	if(gCfgItems.temperature_ctrl_err==1)
    	{
    		//gCfgItems.temperature_ctrl_err = 0;
    		sd.sdmode = 0;
			GUI_SetBkColor(gCfgItems.background_color);
			GUI_SetColor(0X0000ff);//	红色字体
			GUI_DispStringAt("Error",260, 0);
			GUI_SetBkColor(gCfgItems.background_color);
			GUI_SetColor(gCfgItems.title_color);			
    	}
    	//关机
    	Close_machine_signal();
		//手动调平
		//HAL::Leveling_move_action();
		//换料
		//FilamentChange_handle();
		//打印完成之后，返回正在打印界面
    
    	if(Back_to_Printing == 1)
    	{
    		Back_to_Printing = 0;
    		clear_cur_ui();
			draw_printing();
...
static uint8_t touch_exe_freq = 0;
void Commands::checkForPeriodicalActions(bool allowNewMoves) {
    //tan 2016.07.11
    
    touch_exe_freq++;
    if(touch_exe_freq >= 100)//稍微延时。
    {
    	touch_exe_freq -= 100;
    	GUI_TOUCH_Exec();
    }
...
void Commands::waitUntilEndOfAllMoves() {
    ...
    while(PrintLine::hasLines()) {
        ...
        UI_MEDIUM;
        UI_SLOW;
        HAL_IWDG_Refresh(&hiwdg);       //长距离移动时喂狗
    }
}
...
if(CfgPrinterItems.cfg_have_heated_bed == 1)
{
    ...
    if(showRaw) {
        Com::printF(Com::tSpaceRaw,(int)CfgPrinterItems.cfg_num_extruder);
...
if((CfgPrinterItems.cfg_num_extruder>1)&&(MIXING_EXTRUDER == 0))
{
...
//#elif NUM_EXTRUDER == 1
else if(CfgPrinterItems.cfg_num_extruder == 1)
{
    if(showRaw) {
...
#ifdef MOTOR_CURRENT_PERCENT
    const float digipot_motor_current[] = MOTOR_CURRENT_PERCENT;
    for(int i = 0; i < CfgPrinterItems.cfg_num_extruder+3; i++)
        setMotorCurrentPercent(i,digipot_motor_current[i]);
#else
    const uint8_t digipot_motor_current[] = MOTOR_CURRENT;
    for(uint8_t i = 0; i < CfgPrinterItems.cfg_num_extruder+3; i++)
        setMotorCurrent(i,digipot_motor_current[i]);
#endif
}
#endif
...
//#if NUM_EXTRUDER > 1
if((FEATURE_RETRACTION)&&(CfgPrinterItems.cfg_num_extruder>0))
{
if(CfgPrinterItems.cfg_num_extruder > 1)
{
...
//#endif // FEATURE_RETRACTION
        ...
        case 28: { //G28 Home all Axis one at a time
                ...
                {
                	Printer::homeAxis_delta(homeAllAxis || com->hasX(),homeAllAxis || com->hasY(),homeAllAxis || com->hasZ());
                }
				else
				{
...
else
{
                        ...
                        Printer::zLength = zup + sum - Change_Repetier_Define.def_ENDSTOP_Z_BACK_ON_HOME;
}
//#endif // DELTA
...
//#if DRIVE_SYSTEM == DELTA
                    ...
                    if (com->hasX() || com->hasY() || com->hasZ())
                        ...
                        if (Printer::currentPosition[X_AXIS] == 0
                            ...
                            if(Printer::isLargeMachine()) {
                                ...
                                h -= RMath::sqr(bSteps);
                                //h = SQRT(h);
                                h = (unsigned long)SQRT((long double)h);       //delta-skyblue 
                                EEPROM::setRodRadius(h*Printer::invAxisStepsPerMM[Z_AXIS]);
                            }
                        } else
        ...
        case 133: { // G133 Measure steps to top
                ...
                PrintLine::moveRelativeDistanceInSteps(0, 0, Printer::axisStepsPerMM[Z_AXIS] * -Change_Repetier_Define.def_ENDSTOP_Z_BACK_MOVE, 0, Printer::homingFeedrate[Z_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR, true, false);
                Printer::homeAxis_delta(true,true,true);
            }
            break;
        case 135: // G135
           // Com::printF(PSTR("CompDelta:"),Printer::currentNonlinearPositionSteps[A_TOWER]);  //delta-skyblue
           // Com::printF(Com::tComma,Printer::currentNonlinearPositionSteps[B_TOWER]);         //delta-skyblue
           // Com::printFLN(Com::tComma,Printer::currentNonlinearPositionSteps[C_TOWER]);       //delta-skyblue        
...
//#if FEATURE_Z_PROBE //&& NUM_EXTRUDER > 1
        case 134: 
			...
			{ // - G134 Px Sx Zx - Calibrate nozzle height difference (need z probe in nozzle!) Px = reference extruder, Sx = only measure extrude x against reference, Zx = add to measured z distance for Sx for correction.
                ...
                float mins[6],maxs[6],avg[6];
				for(int i = 0; i < CfgPrinterItems.cfg_num_extruder; i++) { // silence unnecessary compiler warning
				      avg[i] = 0;
				}
                bool bigError = false;
...
#define G134_PRECISION 0.05
                ...
                for(int r = 0; r < G134_REPETITIONS && !bigError; r++) {
                    ...
                    for(int i = 0; i < CfgPrinterItems.cfg_num_extruder && !bigError; i++) {
                        if(i == p) continue;
                        if(s >= 0 && i != s) continue;
    ...
    switch( com->M ) {
        ...
        case 23: //M23 - Select file
            if(com->hasString()) {
                ...
                //strcpy((char *)sd.gCurDir, "1:");
                strcpy((char *)sd.gCurDir, SD_Path);
                strcat(sd.gCurDir,com->text);
                strcpy(com->text,sd.gCurDir);
...
#endif
            if (com->hasS()) {
                if(com->hasT() && com->T < CfgPrinterItems.cfg_num_extruder)
...
//#if NUM_EXTRUDER > 0
			if(CfgPrinterItems.cfg_num_extruder>0)
            {
                ...
                if(com->hasT() && com->T < CfgPrinterItems.cfg_num_extruder) actExtruder = &extruder[com->T];
...
//#endif
        ...
        case 116: // Wait for temperatures to reach target temperature
        	if(CfgPrinterItems.cfg_have_heated_bed == 1)
        	{
	            for(fast8_t h = 0; h <= CfgPrinterItems.cfg_num_extruder; h++) {
	                EVENT_WAITING_HEATER(h < CfgPrinterItems.cfg_num_extruder ? h : -1);
	                tempController[h]->waitForTargetTemperature();
	                EVENT_HEATING_FINISHED(h < CfgPrinterItems.cfg_num_extruder ? h : -1);
	            }
        	}
			else
			{
				for(fast8_t h = 0; h <= (CfgPrinterItems.cfg_num_extruder-1); h++) {
	                EVENT_WAITING_HEATER(h < CfgPrinterItems.cfg_num_extruder ? h : -1);
	                tempController[h]->waitForTargetTemperature();
	                EVENT_HEATING_FINISHED(h < CfgPrinterItems.cfg_num_extruder ? h : -1);
	            }
...
if(CfgPrinterItems.cfg_have_heated_bed != 0)
{
...
//#endif
        ...
        case 163: // M163 S<extruderNum> P<weight>  - Set weight for this mixing extruder drive
            if(com->hasS() && com->hasP() && com->S < CfgPrinterItems.cfg_num_extruder && com->S >= 0)
        ...
        case 204: { // M204
                ...
                if(com->hasS()) {
                    if(com->S < 0) break;
                    if(com->S < CfgPrinterItems.cfg_num_extruder) temp = &extruder[com->S].tempControl;
//#if HAVE_HEATED_BED
                    else 
                    {
						if(CfgPrinterItems.cfg_have_heated_bed == 1)
...
//#endif
        ...
        case 280: // M280
			...
			if(CfgPrinterItems.cfg_drive_system == DELTA)
			{
				Printer::homeXAxis_delta();
			}
			else if(CfgPrinterItems.cfg_drive_system == TUGA)
			{
        ...
        case 303: { // M303
				...
				if(CfgPrinterItems.cfg_have_heated_bed == 1)
				{
                	if(cont >= CfgPrinterItems.cfg_num_extruder) cont = CfgPrinterItems.cfg_num_extruder;
				}
				else
				{
					if(cont >= (CfgPrinterItems.cfg_num_extruder-1)) cont = CfgPrinterItems.cfg_num_extruder-1;
				}
				if(cont < 0) cont = 0;
        ...
        case 600:
            //uid.executeAction(UI_ACTION_WIZARD_FILAMENTCHANGE, true); //mask by skyblue
            break;
#endif
        case 601:
            if(com->hasS() && com->S > 0)
...
if((EXT0_HEATER_PIN > -1)&&(CfgPrinterItems.cfg_num_extruder>0))
    WRITE(EXT0_HEATER_PIN,HEATER_PINS_INVERTED);
//#endif
//#if defined(EXT1_HEATER_PIN) && EXT1_HEATER_PIN > -1 && NUM_EXTRUDER > 1
if ((EXT1_HEATER_PIN > -1) &&( CfgPrinterItems.cfg_num_extruder > 1))
    WRITE(EXT1_HEATER_PIN, HEATER_PINS_INVERTED);
//#endif
//#if defined(EXT2_HEATER_PIN) && EXT2_HEATER_PIN > -1 && NUM_EXTRUDER > 2
if (( EXT2_HEATER_PIN > -1) && (CfgPrinterItems.cfg_num_extruder > 2))
    WRITE(EXT2_HEATER_PIN, HEATER_PINS_INVERTED);
//#endif
//#if defined(EXT3_HEATER_PIN) && EXT3_HEATER_PIN > -1 && NUM_EXTRUDER > 3
if ((EXT3_HEATER_PIN > -1) && (CfgPrinterItems.cfg_num_extruder > 3))
    WRITE(EXT3_HEATER_PIN, HEATER_PINS_INVERTED);
//#endif
//#if defined(EXT4_HEATER_PIN) && EXT4_HEATER_PIN > -1 && NUM_EXTRUDER > 4
if ((EXT4_HEATER_PIN > -1) && (CfgPrinterItems.cfg_num_extruder > 4))
    WRITE(EXT4_HEATER_PIN, HEATER_PINS_INVERTED);
//#endif
//#if defined(EXT5_HEATER_PIN) && EXT5_HEATER_PIN > -1 && NUM_EXTRUDER > 5
if ((EXT5_HEATER_PIN > -1) && (CfgPrinterItems.cfg_num_extruder > 5))
    WRITE(EXT5_HEATER_PIN, HEATER_PINS_INVERTED);
//#endif
#if FAN_PIN > -1 && FEATURE_FAN_CONTROL
    WRITE(FAN_PIN, 0);
    //FAN_OP=0;	//skyblue 2016-11-26
#endif
//#if HAVE_HEATED_BED && HEATED_BED_HEATER_PIN > -1
if((CfgPrinterItems.cfg_have_heated_bed == 1)&&(HEATED_BED_HEATER_PIN > -1))
{
