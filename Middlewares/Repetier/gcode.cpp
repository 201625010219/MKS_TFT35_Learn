...
void GCode::readFromSerial()
{
    ...
    while( sd.filesize > sd.sdpos && commandsReceivingWritePosition < MAX_CMD_SIZE)    // consume data until no data or buffer full
    {
        ...
        //int n = sd.file.read();
        int n = -1;
		    f_read(&sd.curFile, (uint8_t *)&n, 1, &len);
    ...
    Com::printF(PSTR("mks Done"));    
    Com::println();              
#if 1
	if(CfgPrinterItems.cfg_print_auto_close == 1)
	{
		CfgPrinterItems.cfg_print_auto_close = 0;
