...
float maxadvspeed = 0;
#endif
uint8_t pwm_pos[11]; // 0-NUM_EXTRUDER = Heater 0-NUM_EXTRUDER of extruder, NUM_EXTRUDER = Heated bed, NUM_EXTRUDER+1 Board fan, NUM_EXTRUDER+2 = Fan
...
{
    if(Printer::debugDryrun() || (CfgPrinterItems.cfg_ext_mintemp> 30 && Extruder::current->tempControl.currentTemperatureC < CfgPrinterItems.cfg_ext_mintemp && !Printer::isColdExtrusionAllowed()))
...
//#endif
    ...
    timeInTicks = (int32_t)timeForMove;
    UI_MEDIUM; // do check encoder
    // Compute the slowest allowed interval (ticks/step), so maximum feedrate is not violated
	int32_t limitInterval0;
    int32_t limitInterval = limitInterval0 = (int32_t)(timeForMove / stepsRemaining); // until not violated by other constraints it is your target speed
...
//#if (DRIVE_SYSTEM == DELTA) // No point computing Z Jerk separately for delta moves
    ...
    float jerk_delta = maxJoinSpeed * lengthFactor * (1.0 - (current->speedX * previous->speedX + current->speedY * previous->speedY + current->speedZ * previous->speedZ) / (current->fullSpeed * previous->fullSpeed));
#else
    float dx_delta = current->speedX - previous->speedX;
    float dy_delta = current->speedY - previous->speedY;
    float dz_delta = current->speedZ - previous->speedZ;
    float jerk_delta = sqrt(dx_delta * dx_delta + dy_delta * dy_delta + dz_delta * dz_delta) * lengthFactor;
...
//#endif // DELTA
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
	    if(jerk_delta > Printer::maxJerk) {
	        factor = Printer::maxJerk / jerk_delta; // always < 1.0!
...
//#if NONLINEAR_SYSTEM
		uint8_t w;
if(CfgPrinterItems.cfg_drive_system == 1)
{
        w = 3;
}
//#else
...
uint8_t transformCartesianStepsToDeltaSteps(int32_t cartesianPosSteps[], int32_t corePosSteps[])
{
	...
	else if(CfgPrinterItems.cfg_drive_system == YX_GANTRY)
	{
	...
	else if(CfgPrinterItems.cfg_drive_system == ZX_GANTRY)
	{
	...
	else if(CfgPrinterItems.cfg_drive_system == GANTRY_FAKE)
	{
...
*/
uint8_t transformCartesianStepsToDeltaSteps(long cartesianPosSteps[], long deltaPosSteps[])     //delta-skyblue
{
    ...
    if(Printer::isLargeMachine())
    {
        ...
        // A TOWER height
        //uint64_t temp = RMath::absLong(Printer::deltaAPosYSteps - cartesianPosSteps[Y_AXIS]); //delta-skyblue
        //uint64_t opt = Printer::deltaDiagonalStepsSquaredA.L;                                 //delta-skyblue
        long double temp = RMath::absLong(Printer::deltaAPosYSteps - cartesianPosSteps[Y_AXIS]); 
        long double opt = Printer::deltaDiagonalStepsSquaredA.L;                                 
    ...
    else
    {
        ...
        long double temp = RMath::absLong(Printer::deltaAPosYSteps - cartesianPosSteps[Y_AXIS]);        //delta-skybue
        long double opt = Printer::deltaDiagonalStepsSquaredA.l;                                        //delta-skybue
        ...
        temp *= temp;
        if (opt < temp)
            RETURN_0("Apos x square "); //delta err 1
...
//#endif
        ...
        else
        {
            ...
            if(CfgPrinterItems.cfg_drive_system == TUGA)
            {
            	Com::printWarningF(Com::tInvalidDeltaCoordinate_tuga);
            }
			else if(CfgPrinterItems.cfg_drive_system == DELTA)
			{
				Com::printWarningF(Com::tInvalidDeltaCoordinate_delta);
			}
			else
			{
...
//#if DRIVE_SYSTEM == DELTA	
    float feedrate_delta = RMath::min(Printer::feedrate, Printer::maxFeedrate[Z_AXIS]);
//#else
...
//#endif	
    if (cartesianDir & XY_STEP)
    {
        ...
        float seconds_delta= cartesianDistance / feedrate_delta;
		float seconds= cartesianDistance / feedrate;
#ifdef DEBUG_SPLIT
		if(CfgPrinterItems.cfg_drive_system == DELTA)
		{
			Com::printFLN(Com::tDBGDeltaSeconds, seconds_delta);
		}
		else
        ...
        if(CfgPrinterItems.cfg_drive_system == DELTA)
        {
        	segmentCount = RMath::max((int16_t)1, static_cast<int16_t>(sps * seconds_delta));      //delta-skyblue
        }
		else
        	segmentCount = RMath::max((int16_t)1, static_cast<int16_t>(sps * seconds));      //delta-skyblue
    ...
    for (int lineNumber = 1; lineNumber <= numLines; lineNumber++)
    {
        ...
        if (virtualAxisSteps == 0 && p->delta[E_AXIS] == 0)
        {
            if (numLines != 1)
            {
            	if(CfgPrinterItems.cfg_drive_system == TUGA)
            	{
            		Com::printErrorFLN(Com::tDBGDeltaNoMoveinDSegment_tuga);
            	}
				else if(CfgPrinterItems.cfg_drive_system == DELTA)
				{
					Com::printErrorFLN(Com::tDBGDeltaNoMoveinDSegment_delta);
				}
				else
...
int32_t PrintLine::bresenhamStep_nonlinear_system() // Version for delta printer
{
#if CPU_ARCH == ARCH_ARM
    if(!PrintLine::nlFlag)
#else
...
//#if STEPPER_HIGH_DELAY + DOUBLE_STEP_DELAY
		if(CfgPrinterItems.cfg_stepper_high_delay+DOUBLE_STEP_DELAY)
		{
        	if(loop > 0)
            	//HAL::delayMicroseconds(STEPPER_HIGH_DELAY + DOUBLE_STEP_DELAY);
            	HAL::delayMicroseconds(CfgPrinterItems.cfg_stepper_high_delay + DOUBLE_STEP_DELAY);
		}
//#endif
...
int32_t PrintLine::bresenhamStep() // version for Cartesian printer
{
...
#endif
    {
        ...
        if(cur->isBlocked())   // This step is in computation - shouldn't happen
        {
            ...
            {
                HAL::allowInterrupts();	//skyblue 2017-01-10
                lastblk = (int)cur;
                Com::printFLN(Com::tBLK, (int32_t)linesCount);
            }
            cur = NULL;
...
if((CfgPrinterItems.cfg_drive_system == XY_GANTRY)||( CfgPrinterItems.cfg_drive_system == YX_GANTRY))
{
        if(cur->isXOrYMove())
        {
            Printer::enableXStepper();
...
if(CfgPrinterItems.cfg_drive_system == XZ_GANTRY)
...
//#if STEPPER_HIGH_DELAY + DOUBLE_STEP_DELAY > 0
	if(CfgPrinterItems.cfg_stepper_high_delay+DOUBLE_STEP_DELAY > 0)
	{
        if(loop)
            //HAL::delayMicroseconds(STEPPER_HIGH_DELAY + DOUBLE_STEP_DELAY);
            HAL::delayMicroseconds(CfgPrinterItems.cfg_stepper_high_delay + DOUBLE_STEP_DELAY);
...
void PrintLine::motionReset()
{
 ...
 maxadvspeed = 0;
  ...
  waitRelax = 0; // Delay filament relax at the end of print, could be a simple timeout


cur = NULL;               ///< Current printing line
#if CPU_ARCH == ARCH_ARM
nlFlag = false;
#endif
linesWritePos = 0;            ///< Position where we write the next cached line move.
linesCount = 0;      ///< Number of lines cached 0 = nothing to do.
linesPos = 0;                 ///< Position for executing line movement.
