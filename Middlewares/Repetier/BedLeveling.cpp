...
if(CfgPrinterItems.cfg_bed_leveling_method == 0)
{
    float h;
...
else if(CfgPrinterItems.cfg_bed_leveling_method == 1)
{
    //float delta = 1.0 / (BED_LEVELING_GRID_SIZE - 1);
    float delta = 1.0 / (CfgPrinterItems.cfg_bed_leveling_grid_size - 1);
    ...
    for(int ix = 0; ix < CfgPrinterItems.cfg_bed_leveling_grid_size; ix++) {
        //for(int iy = 0; iy < BED_LEVELING_GRID_SIZE; iy++) {
        for(int iy = 0; iy < CfgPrinterItems.cfg_bed_leveling_grid_size; iy++) {
...
else if(CfgPrinterItems.cfg_bed_leveling_method == 2)
{
    float h1,h2,h3,h4;
...
{
	Com::printFLN("#error Unknown bed leveling method");
}
//#endif
...
if(CfgPrinterItems.cfg_drive_system == DELTA)
{
    Printer::homeAxis_delta(true, true, true); // shifting z makes positioning invalid, need to recalibrate
}
//#endif
...
if(Change_Repetier_Define.def_NONLINEAR_SYSTEM == 1)
    startZ = realDeltaPositionSteps[Z_AXIS] = currentNonlinearPositionSteps[Z_AXIS]; // update real
//#endif
