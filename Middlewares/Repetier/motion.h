...
else if(CfgPrinterItems.cfg_drive_system == XZ_GANTRY || CfgPrinterItems.cfg_drive_system == ZX_GANTRY)
		dir &= ~80;
//#else
else
        dir &= ~16;
...
if((CfgPrinterItems.cfg_drive_system == XY_GANTRY)||(CfgPrinterItems.cfg_drive_system == XZ_GANTRY))
{
        if(isXPositiveMove())
        {
            Printer::motorX++;
...
if((CfgPrinterItems.cfg_drive_system == YX_GANTRY) || (CfgPrinterItems.cfg_drive_system == ZX_GANTRY))
{
        if(isXPositiveMove())
        {
            Printer::motorX++;
...
//#endif
    ...
    INLINE void startYStep()
		...
		if(CfgPrinterItems.cfg_drive_system == YX_GANTRY)
		{
				if(isYPositiveMove())
				{
					Printer::motorX++;
		...
		if((!( CfgPrinterItems.cfg_drive_system==XY_GANTRY 
				...
				|| CfgPrinterItems.cfg_drive_system==GANTRY_FAKE))
				|| CfgPrinterItems.cfg_drive_system == YX_GANTRY 
				|| CfgPrinterItems.cfg_drive_system == XY_GANTRY)
		{
				Printer::startZStep();
		}
		//#else
		...
		if(CfgPrinterItems.cfg_drive_system == ZX_GANTRY)
		{
				if(isZPositiveMove())
				{
					Printer::motorX++;
...
//#endif
    ...
    static int32_t bresenhamStep_nonlinear_system();
