...
public:
    ...
    static long currentPositionSteps[E_AXIS_ARRAY];     ///< Position in steps from origin.  //delta-skyblue
...
//#if NONLINEAR_SYSTEM
    ...
    static long currentNonlinearPositionSteps[E_TOWER_ARRAY];        //delta-skyblue
...
//#endif
    ...
    static INLINE void setXDirection(bool positive)
    {
        if(positive)
        {
            ...
            //WRITE(X_DIR_PIN,!CfgPrinterItems.cfg_invert_x_dir);
          XDIR_OP = !CfgPrinterItems.cfg_invert_x_dir;
#if FEATURE_TWO_XSTEPPER || DUAL_X_AXIS
            //WRITE(X2_DIR_PIN,!INVERT_X_DIR);
            WRITE(X2_DIR_PIN,!CfgPrinterItems.cfg_invert_x_dir);
#endif
        }
        else
        {
    ...
    static INLINE void setYDirection(bool positive)
    {
        if(positive)
        {
            ...
            //WRITE(Y_DIR_PIN, !CfgPrinterItems.cfg_invert_y_dir);
            YDIR_OP = !CfgPrinterItems.cfg_invert_y_dir;
#if FEATURE_TWO_YSTEPPER
            //WRITE(Y2_DIR_PIN, !INVERT_Y_DIR);
            WRITE(Y2_DIR_PIN, !CfgPrinterItems.cfg_invert_y_dir);
#endif
        }
        else
        {
    ...
    static INLINE void setZDirection(bool positive)
    {
        if(positive)
        {
            ...
            //WRITE(Z_DIR_PIN, !CfgPrinterItems.cfg_invert_z_dir);
            ZDIR_OP = !CfgPrinterItems.cfg_invert_z_dir;
#if FEATURE_TWO_ZSTEPPER
            //WRITE(Z2_DIR_PIN, !INVERT_Z_DIR);
            WRITE(Z2_DIR_PIN, !CfgPrinterItems.cfg_invert_z_dir);
#endif
#if FEATURE_THREE_ZSTEPPER
            //WRITE(Z3_DIR_PIN, !INVERT_Z_DIR);
            WRITE(Z3_DIR_PIN, !CfgPrinterItems.cfg_invert_z_dir);
#endif
        }
        else
        {
    ...
    static INLINE bool getZDirection()
    {
        ...
        //return ((READ(Z_DIR_PIN) != 0) ^ CfgPrinterItems.cfg_invert_z_dir);
        return ((ZDIR_IP != 0) ^ CfgPrinterItems.cfg_invert_z_dir);     //skyblue 2016-12-15 ???
    }

    static INLINE bool getYDirection()
    {
        //return((READ(Y_DIR_PIN) != 0) ^ INVERT_Y_DIR);
        //return((READ(Y_DIR_PIN) != 0) ^ CfgPrinterItems.cfg_invert_y_dir);
        return((YDIR_IP != 0) ^ CfgPrinterItems.cfg_invert_y_dir);      //skyblue 2016-12-15 ???
    }

    static INLINE bool getXDirection()
    {
        //return((READ(X_DIR_PIN) != 0) ^ INVERT_X_DIR);
        //return((READ(X_DIR_PIN) != 0) ^ CfgPrinterItems.cfg_invert_x_dir);
        return((XDIR_IP != 0) ^ CfgPrinterItems.cfg_invert_x_dir);      //skyblue 2016-12-15 ???
    }
    ...
    static INLINE void unsetAllSteppersDisabled()
    {
        flag0 &= ~PRINTER_FLAG0_STEPPER_DISABLED;
#if FAN_BOARD_PIN > -1
        pwm_pos[Change_Repetier_Define.def_PWM_BOARD_FAN] = BOARD_FAN_SPEED;
#endif // FAN_BOARD_PIN
    }
...
if((CfgPrinterItems.cfg_drive_system == XZ_GANTRY)||(CfgPrinterItems.cfg_drive_system == ZX_GANTRY))
{
...
//#if STEPPER_HIGH_DELAY>0
		if(CfgPrinterItems.cfg_stepper_high_delay>0)	
		{
        //HAL::delayMicroseconds(STEPPER_HIGH_DELAY);
        HAL::delayMicroseconds(CfgPrinterItems.cfg_stepper_high_delay);
		}
//#endif
    }
    ...
    static void homeAxis_delta(bool xaxis,bool yaxis,bool zaxis); /// Home axis
    static void setOrigin(float xOff,float yOff,float zOff);
    static bool isPositionAllowed(float x,float y,float z);
    static INLINE int getFanSpeed()
    {
        return (int)pwm_pos[Change_Repetier_Define.def_PWM_FAN1];
    }
    static INLINE int getFan2Speed()
    {
	    return (int)pwm_pos[Change_Repetier_Define.def_PWM_FAN2];
    }
//#if NONLINEAR_SYSTEM
...
//#endif
    ...
    static void printerReset();
		static void homeXAxis_delta();
    static void homeYAxis_delta();
    static void homeZAxis_delta();
};
