...
void EEPROM::restoreEEPROMSettingsFromConfiguration()
{
    ...
    if(newcheck != HAL::eprGetByte(EPR_INTEGRITY_BYTE))
		HAL::eprSetByte(EPR_INTEGRITY_BYTE, newcheck);	
    baudrate = BAUDRATE;
    //baudrate = CfgPrinterItems.cfg_baudrate;
    maxInactiveTime = MAX_INACTIVE_TIME * 1000L;
    stepperInactiveTime = STEPPER_INACTIVE_TIME * 1000L;
    Printer::axisStepsPerMM[X_AXIS] = CfgPrinterItems.cfg_xaxis_steps_per_mm;
    Printer::axisStepsPerMM[Y_AXIS] = CfgPrinterItems.cfg_yaxis_steps_per_mm;
    Printer::axisStepsPerMM[Z_AXIS] = CfgPrinterItems.cfg_zaxis_steps_per_mm;
    Printer::axisStepsPerMM[E_AXIS] = 1;
    Printer::maxFeedrate[X_AXIS] = CfgPrinterItems.cfg_max_feedrate_x;
    Printer::maxFeedrate[Y_AXIS] = CfgPrinterItems.cfg_max_feedrate_y;
    Printer::maxFeedrate[Z_AXIS] = CfgPrinterItems.cfg_max_feedrate_z;
    Printer::homingFeedrate[X_AXIS] = CfgPrinterItems.cfg_homing_feedrate_x;
    Printer::homingFeedrate[Y_AXIS] = CfgPrinterItems.cfg_homing_feedrate_y;
    Printer::homingFeedrate[Z_AXIS] = CfgPrinterItems.cfg_homing_feedrate_z;
...
//#endif
    ...
    Printer::maxAccelerationMMPerSquareSecond[X_AXIS] = CfgPrinterItems.cfg_max_acceleration_units_per_sq_second_x;
    Printer::maxAccelerationMMPerSquareSecond[Y_AXIS] = CfgPrinterItems.cfg_max_acceleration_units_per_sq_second_y;
    Printer::maxAccelerationMMPerSquareSecond[Z_AXIS] = CfgPrinterItems.cfg_max_acceleration_units_per_sq_second_z;
    Printer::maxTravelAccelerationMMPerSquareSecond[X_AXIS] = CfgPrinterItems.cfg_max_travel_acceleration_units_per_sq_second_x;
    Printer::maxTravelAccelerationMMPerSquareSecond[Y_AXIS] = CfgPrinterItems.cfg_max_travel_acceleration_units_per_sq_second_y;
    Printer::maxTravelAccelerationMMPerSquareSecond[Z_AXIS] = CfgPrinterItems.cfg_max_travel_acceleration_units_per_sq_second_z;
#endif
if(CfgPrinterItems.cfg_have_heated_bed == 1)
{
    ...
    Printer::xLength = CfgPrinterItems.cfg_x_max_length;
    Printer::yLength = CfgPrinterItems.cfg_y_max_length;
    Printer::zLength = CfgPrinterItems.cfg_z_max_length;
    Printer::xMin = CfgPrinterItems.cfg_x_min_pos;
    Printer::yMin = CfgPrinterItems.cfg_y_min_pos;
    Printer::zMin = CfgPrinterItems.cfg_z_min_pos;    
//#if NONLINEAR_SYSTEM
if(Change_Repetier_Define.def_NONLINEAR_SYSTEM == 1)
{
//#ifdef ROD_RADIUS
	if(CfgPrinterItems.cfg_drive_system == DELTA)
		Printer::radius0 = CfgPrinterItems.cfg_rod_radius;//ROD_RADIUS;
//#else
	else
		Printer::radius0 = 0;
//#endif
...
{
    e = &extruder[0];
    e->stepsPerMM = CfgPrinterItems.cfg_ext0_steps_per_mm;
    e->maxFeedrate = CfgPrinterItems.cfg_max_feedrate_e0;
    e->maxStartFeedrate = EXT0_MAX_START_FEEDRATE;
    e->maxAcceleration = CfgPrinterItems.cfg_max_acceleration_e0;
...
//#if NUM_EXTRUDER>1
	if(CfgPrinterItems.cfg_num_extruder>1)
		{
    e = &extruder[1];
    e->stepsPerMM = CfgPrinterItems.cfg_ext1_steps_per_mm;//EXT1_STEPS_PER_MM;
    e->maxFeedrate = CfgPrinterItems.cfg_max_feedrate_e1;//EXT1_MAX_FEEDRATE;
    e->maxStartFeedrate = EXT1_MAX_START_FEEDRATE;
    e->maxAcceleration = CfgPrinterItems.cfg_max_acceleration_e1;//EXT1_MAX_ACCELERATION;
...
{
    HAL::eprSetFloat(EPR_Z_PROBE_HEIGHT,CfgPrinterItems.cfg_z_probe_height);
    //HAL::eprSetFloat(EPR_Z_PROBE_SPEED,Z_PROBE_SPEED);
    HAL::eprSetFloat(EPR_Z_PROBE_SPEED,CfgPrinterItems.cfg_z_probe_speed);
    //HAL::eprSetFloat(EPR_Z_PROBE_XY_SPEED,Z_PROBE_XY_SPEED);
    HAL::eprSetFloat(EPR_Z_PROBE_XY_SPEED,CfgPrinterItems.cfg_z_probe_xy_speed);
    ...
    HAL::eprSetFloat(EPR_Z_PROBE_X1,CfgPrinterItems.cfg_z_probe_x1);
    HAL::eprSetFloat(EPR_Z_PROBE_Y1,CfgPrinterItems.cfg_z_probe_y1);
    HAL::eprSetFloat(EPR_Z_PROBE_X2,CfgPrinterItems.cfg_z_probe_x2);
    HAL::eprSetFloat(EPR_Z_PROBE_Y2,CfgPrinterItems.cfg_z_probe_y2);
    HAL::eprSetFloat(EPR_Z_PROBE_X3,CfgPrinterItems.cfg_z_probe_x3);
    HAL::eprSetFloat(EPR_Z_PROBE_Y3,CfgPrinterItems.cfg_z_probe_y3);
...
if(CfgPrinterItems.cfg_drive_system == DELTA)
{
    HAL::eprSetFloat(EPR_DELTA_DIAGONAL_ROD_LENGTH,Change_Repetier_Define.def_DELTA_DIAGONAL_ROD);
    HAL::eprSetFloat(EPR_DELTA_HORIZONTAL_RADIUS,CfgPrinterItems.cfg_rod_radius);//ROD_RADIUS
    HAL::eprSetInt16(EPR_DELTA_TOWERX_OFFSET_STEPS,Change_Repetier_Define.def_DELTA_X_ENDSTOP_OFFSET_STEPS);
    HAL::eprSetInt16(EPR_DELTA_TOWERY_OFFSET_STEPS,Change_Repetier_Define.def_DELTA_Y_ENDSTOP_OFFSET_STEPS);
    HAL::eprSetInt16(EPR_DELTA_TOWERZ_OFFSET_STEPS,Change_Repetier_Define.def_DELTA_Z_ENDSTOP_OFFSET_STEPS);
    HAL::eprSetFloat(EPR_DELTA_ALPHA_A,Change_Repetier_Define.def_DELTA_ALPHA_A);
    HAL::eprSetFloat(EPR_DELTA_ALPHA_B,Change_Repetier_Define.def_DELTA_ALPHA_B);
    HAL::eprSetFloat(EPR_DELTA_ALPHA_C,Change_Repetier_Define.def_DELTA_ALPHA_C);
    HAL::eprSetFloat(EPR_DELTA_RADIUS_CORR_A,Change_Repetier_Define.def_DELTA_RADIUS_CORRECTION_A);
    HAL::eprSetFloat(EPR_DELTA_RADIUS_CORR_B,Change_Repetier_Define.def_DELTA_RADIUS_CORRECTION_B);
    HAL::eprSetFloat(EPR_DELTA_RADIUS_CORR_C,Change_Repetier_Define.def_DELTA_RADIUS_CORRECTION_C);
    HAL::eprSetFloat(EPR_DELTA_MAX_RADIUS,Change_Repetier_Define.def_DELTA_MAX_RADIUS);
    HAL::eprSetFloat(EPR_DELTA_DIAGONAL_CORRECTION_A,Change_Repetier_Define.def_DELTA_DIAGONAL_CORRECTION_A);
    HAL::eprSetFloat(EPR_DELTA_DIAGONAL_CORRECTION_B,Change_Repetier_Define.def_DELTA_DIAGONAL_CORRECTION_B);
    HAL::eprSetFloat(EPR_DELTA_DIAGONAL_CORRECTION_C,Change_Repetier_Define.def_DELTA_DIAGONAL_CORRECTION_C);
}
//#endif
    HAL::eprSetFloat(EPR_AXISCOMP_TANXY,AXISCOMP_TANXY);
    HAL::eprSetFloat(EPR_AXISCOMP_TANYZ,AXISCOMP_TANYZ);
...
//#endif
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
		writeFloat(EPR_ZAXIS_STEPS_PER_MM, Com::tEPRZStepsPerMM_delta, 4);
	}
	else
...
//#endif
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
		writeFloat(EPR_Z_MAX_FEEDRATE, Com::tEPRZMaxFeedrate_delta);
	}
	else
	{
...
//#endif
	if(CfgPrinterItems.cfg_drive_system == DELTA)
	{
		writeFloat(EPR_Z_HOMING_FEEDRATE, Com::tEPRZHomingFeedrate_delta);
	}
	else
	{
...
//#endif
    ...
    writeFloat(EPR_DELTA_DIAGONAL_ROD_LENGTH, Com::tEPRDiagonalRodLength_tuga);
#endif
//#if DRIVE_SYSTEM == DELTA
if(CfgPrinterItems.cfg_drive_system == DELTA)
{
    writeFloat(EPR_Z_MAX_ACCEL, Com::tEPRZAcceleration_delta);
    writeFloat(EPR_Z_MAX_TRAVEL_ACCEL, Com::tEPRZTravelAcceleration_delta);
...
#endif
    writeFloat(EPR_DELTA_DIAGONAL_ROD_LENGTH, Com::tEPRDiagonalRodLength_delta);
    writeFloat(EPR_DELTA_HORIZONTAL_RADIUS, Com::tEPRHorizontalRadius_delta);
    writeFloat(EPR_DELTA_MAX_RADIUS, Com::tEPRDeltaMaxRadius_delta);
    writeInt(EPR_DELTA_TOWERX_OFFSET_STEPS, Com::tEPRTowerXOffset_delta);
    writeInt(EPR_DELTA_TOWERY_OFFSET_STEPS, Com::tEPRTowerYOffset_delta);
    writeInt(EPR_DELTA_TOWERZ_OFFSET_STEPS, Com::tEPRTowerZOffset_delta);
...
{
    for(uint8_t e = 0; e < CfgPrinterItems.cfg_num_extruder; e++)
    {
