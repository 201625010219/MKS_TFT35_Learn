...
/*********************change*********************/
		...
		//**if(wifi_link_state != WIFI_TRANS_FILE)
	  	//**{
  			...
  			{
  				//console 
  				switch(console_flag)       	//状态机，使用console_flag判定WIFI状态，并执行相应状态的动作
                {							//默认开机console_flag = 0;
                    case 1:					//状态0：在switch中进入default，无动作执行，直接break;
                      console_show(); 		//状态1：执行console_show(),并标志成状态3;
                      console_flag=3;		//状态2：执行console_back(),并标志成状态0; 
                        break;				//状态3：判定update_msg_flag是否为1, update_msg_flag = 1,执行console_show,且update_msg_flag = 0
                    case 2:
                      console_back();  
                      console_flag=0;
					 lineNumber = 0;
